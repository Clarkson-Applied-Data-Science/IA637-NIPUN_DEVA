...
class baseObject:
    ...
    def deleteByField(self, field, val):
        try:
            sql = f"Delete From `{self.tn}` where `{field}` = %s"
            self.cur.execute(sql, (val))
        except pymysql.err.IntegrityError as e:
            print("IntegrityError: Unable to delete due to constraints.")
        except Exception as e:
            print("Check the tables on your db")
